 ...
 */
public class CLParallelOSMLocalMem {
	...
	private int maxNumberOfElementsPerCell;
	...
	public List<VPoint> update() throws OpenCLException {
		try (MemoryStack stack = stackPush()) {
			...
			int[] start = CLUtils.toIntArray(memStart, numberOfGridCells);
			int[] end = CLUtils.toIntArray(memEnd, numberOfGridCells);
	...
	private void clSeek(
		...
		try (MemoryStack stack = stackPush()) {
			...
			long maxWorkGroupSize = CLUtils.getMaxWorkGroupSizeForKernel(clDevice, clSeek, 4 * maxNumberOfElementsPerCell * 9, max_work_group_size, max_local_memory_size); // local 4 byte (integer)
	...
	private void buildProgram() throws OpenCLException {
		try (MemoryStack stack = stackPush()) {
			...
			ByteBuffer source;
			try {
				source = CLUtils.ioResourceToByteBuffer("ParallelOSM_localMem.cl", 4096);
			} catch (IOException e) {
